...
const RouteEditor: React.FC = () => {
  ...
  useEffect(() => {
        ...
        if (editingRoute && event.latLng) {
          ...
          const geocoder = new window.google.maps.Geocoder();
            ...
            (
              ...
              // If geocoding fails, still allow adding stop with just lat/lng
              setNewStop({
                name: '',
                address: `${lat.toFixed(4)}, ${lng.toFixed(4)}`, // Default to lat/lng if address not found
                lat,
                lng,
                estimatedTime: 15,
                wasteType: 'residential',
    ...
    if (window.google) {
      ...
      script.src = `https://maps.googleapis.com/maps/api/js?key=AIzaSyB7W5khIwaVuZDmQg5ebdTvd3PEHv0ALbs&libraries=places`; // Replace YOUR_API_KEY
      script.async = true;
      script.defer = true;
  ...
  useEffect(() => {
    ...
    } else if (map && !editingRoute) {
      // If no route is being edited, clear the map
      directionsRenderer?.setDirections(null);
      markers.forEach(marker => marker.setMap(null));
      setMarkers([]);
      map.setCenter({ lat: 39.4699, lng: -0.3763 }); // Center to a default location
      map.setZoom(13);
    }
  }, [map, editingRoute, directionsRenderer]); // Add directionsRenderer as a dependency

  const displayRouteOnMap = (route: RouteData) => {
    if (!map || !directionsService || !directionsRenderer) return;

    // Clear existing markers and directions
    directionsRenderer.setDirections(null); // Clear previous directions
    markers.forEach(marker => marker.setMap(null));
    setMarkers([]);
    ...
    if (route.stops.length < 2) {
      ...
      if (route.stops.length === 1) {
        map.setCenter({ lat: route.stops[0].lat, lng: route.stops[0].lng });
        map.setZoom(15);
      } else if (route.stops.length === 0) {
        map.setCenter({ lat: 39.4699, lng: -0.3763 }); // Default center for empty route
        map.setZoom(13);
      }
      return;
    }
    ...
    directionsService.route({
      ...
      if (status === 'OK' && result) {
        ...
        // Fallback: if directions fail, display markers individually
        const newMarkers: google.maps.Marker[] = [];
        route.stops.forEach((stop, index) => {
          const marker = new window.google.maps.Marker({
            position: { lat: stop.lat, lng: stop.lng },
        ...
        setMarkers(newMarkers);
        // Center the map on the first stop if there are stops
  ...
  const handleCreateNewRoute = () => {
    ...
    // Explicitly call displayRouteOnMap for the new empty route
    // The useEffect will handle this, but it's good to be explicit here for clarity if needed
  ...
  const handleAddStop = () => {
    ...
    setEditingRoute(prevRoute => {
      if (prevRoute) {
        const updatedRoute = {
          ...prevRoute,
          stops: [...prevRoute.stops, stop]
        };
        // After updating the stops, re-render the map with the new route
        // This is now handled by the useEffect, so no explicit call needed here
        return updatedRoute;
      }
      return prevRoute;
    });
  ...
  const handleRemoveStop = (stopId: string) => {
    ...
    setEditingRoute(prevRoute => {
      if (prevRoute) {
        const updatedRoute = {
          ...prevRoute,
          stops: prevRoute.stops.filter(s => s.id !== stopId)
        };
        // After removing a stop, re-render the map
        // This is now handled by the useEffect, so no explicit call needed here
        return updatedRoute;
      }
      return prevRoute;
